...
export function StepResults({
  ...
  const [showChangepoints, setShowChangepoints] = useState<boolean>(true);

  const chartRef = useRef<HTMLDivElement>(null);

  // Map historical actuals by date string
  const actualsMap = useMemo(() => {
    const map = new Map<string, number>();
    for (const d of data) {
      if (d.ds) {
        map.set(d.ds.split(" ")[0], d.y);
      }
    }
    return map;
  }, [data]);

  // Memoize forecast chart data (combining forecast yhat with historical actual y)
  const forecastChartData = useMemo(() => {
    if (!forecastResults) return [];
    return forecastResults.forecast.map((pt) => {
      const dateKey = pt.ds.split(" ")[0];
      const actualVal = actualsMap.get(dateKey);
      return {
        ds: dateKey,
        yhat: Number(pt.yhat.toFixed(2)),
        y: actualVal !== undefined ? Number(actualVal.toFixed(2)) : undefined,
        bounds: [
  ...
  const handleExportJSON = () => {
    ...
    link.href = url;
    link.download = `prophetly_forecast_${Date.now()}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success("Exported full dataset & metrics as JSON!");
  };
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.download = `prophetly_chart_${Date.now()}.png`;
      link.href = dataUrl;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const executionTimeMs =
    forecastResults?.executionTimeMs ?? cvResults?.executionTimeMs;
        ...
        <CardContent className="flex-1 overflow-hidden p-4 md:p-6">
          {activeResultsMode === "forecast" && forecastResults ? (
            <Tabs defaultValue="chart" className="flex h-full flex-col">
              ...
              <TabsContent
                value="chart"
                className="flex-1 pt-3 h-full min-h-0 flex flex-col gap-2"
              >
                <div className="flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2">
                    <Button
                      type="button"
                      variant={showChangepoints ? "secondary" : "outline"}
                      size="sm"
                ...
                <div
                  ref={chartRef}
                  className="h-full w-full flex-1 min-h-0 bg-card rounded-lg p-1"
                >
                  <ChartContainer
                    config={FORECAST_CHART_CONFIG}
                    className="h-full w-full"
                    aria-label="Main Prophet forecast chart with upper/lower uncertainty bounds and actuals overlay"
                  >
            ...
            <Tabs defaultValue="overview" className="flex h-full flex-col">
              ...
              >
                <div
                  ref={chartRef}
                  className="h-full w-full bg-card rounded-lg p-1"
                >
                  <ChartContainer
