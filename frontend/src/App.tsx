...
function App() {
  ...
  const { isLoading, setForecast, forecast, clearResults, isWarmingUp, error } = useResultsStore();
  
  const { executeWithRetry, lastErrorWasTimeout } = useRetry();

  // Determine active step from route
  const getStep = () => {
    if (location.pathname === '/config') return 2;
    if (location.pathname === '/results') return 3;
    return 1; // default to upload
  };
  const step = getStep();
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-purple-500/30 flex flex-col items-center p-6 overflow-hidden">
      {/* Progress header */}
      <div className="w-full flex justify-center items-center gap-4 text-sm font-medium text-zinc-500 py-8 mb-4 shrink-0 z-10" aria-label="Progress steps">
        <div className={`flex items-center gap-2 ${step >= 1 ? 'text-zinc-200' : ''}`} aria-current={step === 1 ? 'step' : undefined}>
        ...
        <div className={`flex items-center gap-2 ${step >= 2 ? 'text-zinc-200' : ''}`} aria-current={step === 2 ? 'step' : undefined}>
        ...
        <div className="w-8 h-px bg-zinc-800" />
        <div className={`flex items-center gap-2 ${step >= 3 ? 'text-zinc-200' : ''}`} aria-current={step === 3 ? 'step' : undefined}>
      ...
      <main className="w-full max-w-6xl relative flex-1 flex flex-col">
        ...
        {error && lastErrorWasTimeout && !isWarmingUp && (
        ...
        <Routes>
          ...
          <Route path="/results" element={
            ...
            ) : <Navigate to={isDataValid() ? "/config" : "/upload"} replace />
          } />
        </Routes>
      </main>
