"""File upload API endpoints with privacy-first processing."""
...
async def upload_csv_file(
    ...
    Privacy guarantee: File is processed entirely in memory and never
    written to disk. Data is automatically cleaned up when session expires.
    """
    with MemoryTracker("upload_csv_file"):
        try:
            ...
            logger.info("Processing file upload: %s (session: %s)", file.filename, session_id)
            ...
            quality_assessment = data_quality_service.assess_data_quality(df, processing_result['column_info'])
...
async def get_file_info(session_id: str) -> Dict[str, Any]:
    ...
    with MemoryTracker("get_file_info"):
        ...
        return {
            ...
            'privacy_notice': 'File data exists only in session memory and will be automatically cleaned up'
        }
...
async def get_data_quality_assessment(session_id: str) -> Dict[str, Any]:
    ...
    with MemoryTracker("get_data_quality_assessment"):
        ...
        if quality_assessment is None:
            raise HTTPException(status_code=404, detail="No data quality assessment found in this session")

        return {
            'session_id': session_id,
            'assessment': quality_assessment,
            'privacy_notice': 'Quality assessment performed on session data only - no data stored on server'
        }
...
async def clear_file_data(session_id: str) -> Dict[str, Any]:
    ...
    with MemoryTracker("clear_file_data"):
        ...
        removed_items = []
        for key in ['uploaded_file_data', 'file_metadata', 'column_info', 'data_quality_assessment']:
        ...
        return {
            ...
            'privacy_notice': 'All file data has been securely removed from server memory'
        }
...
async def get_upload_limits() -> Dict[str, Any]:
    ...
    return {
        ...
        'privacy_policy': 'All files are processed in memory only and never stored on disk'
    }
