    ...
    with MemoryTracker("execute_cross_validation"):
        ...
        try:
            ...
            return CrossValidationResult(
                ...
                metrics=metrics,
                results=cv_points,
                cutoff_count=len(cv_results_df['cutoff'].unique()) if cv_results_df is not None else 0,
...
async def validate_cross_validation_config(request: CrossValidationRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Cross-validation config validation completed: valid={cv_validation['is_valid']}")
        return cv_validation

    except Exception as e:
        logger.error(f"Cross-validation config validation failed: {e}", exc_info=True)
        return {
            'is_valid': False,
...
def _validate_cv_config(config: CrossValidationConfig, data: pd.DataFrame) -> None:
    ...
    if initial_days > data_span_days * 0.8:
        raise ValueError(f"Initial period ({initial_days} days) too large for data span ({data_span_days} days)")
...
def _validate_cv_config_detailed(config: CrossValidationConfig, data: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(data) > 10000:
        ...
        result['recommendations'].append(
            "Consider using a subset of data or parallel processing"
        )
    ...
    if cutoff_count < 3:
        result['warnings'].append(
            f"Few cutoffs ({cutoff_count}) may provide limited validation"
        )
        result['recommendations'].append(
...
def _execute_prophet_cross_validation(
    ...
    try:
        ...
        logger.error(f"Prophet cross-validation execution failed: {e}")
...
def _convert_cv_results_to_points(cv_results: pd.DataFrame) -> List[CrossValidationPoint]:
    ...
    for _, row in cv_results.iterrows():
        ...
        points.append(CrossValidationPoint(
            ds=row['ds'].isoformat() if hasattr(row['ds'], 'isoformat') else str(row['ds']),
            cutoff=row['cutoff'].isoformat() if hasattr(row['cutoff'], 'isoformat') else str(row['cutoff']),
            y=float(row['y']),
            yhat=float(row['yhat']),
...
def _estimate_cv_processing_time(data: pd.DataFrame, cv_config: CrossValidationConfig, forecast_config: ForecastConfig) -> float:
    ...
    complexity_factor = 1.0 + len(forecast_config.custom_seasonalities) * 0.1 + len(forecast_config.regressors) * 0.05
    
    return base_time * data_factor * cutoff_factor * mcmc_factor * complexity_factor


def _estimate_cv_memory_usage(data: pd.DataFrame, cv_config: CrossValidationConfig, forecast_config: ForecastConfig) -> float:
