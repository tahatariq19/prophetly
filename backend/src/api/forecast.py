    ...
    with MemoryTracker("generate_forecast"):
        try:
            ...
            try:
                ...
                logger.warning(
                    "Component decomposition failed. See details in next log entry if available."
                )
            ...
            if components_df is not None:
                for i, row in components_df.iterrows():
                    component_data = ComponentData(
                        ds=row["ds"].isoformat(), trend=float(row.get("trend", 0))
                    )
            ...
            if len(data) > 0:
                ...
                performance_metrics = {
                    ...
                    "mape": float(abs(residuals / actual_values).mean() * 100)
                    if (actual_values != 0).all()
                    else None,
                    "r2": float(
                        1
                        - (residuals**2).sum() / ((actual_values - actual_values.mean()) ** 2).sum()
                    ),
                }
            ...
            logger.error(
                "Forecast generation failed. See details in next log entry if available.",
                exc_info=True,
            )
            raise HTTPException(
...
async def validate_forecast_request(request: ForecastRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            "Forecast validation failed. See details in next log entry if available.", exc_info=True
        )
        return {
            "is_valid": False,
